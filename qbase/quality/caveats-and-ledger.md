...
| C3 | `forecast_snap` 91 行(0.07%)`first_ann_date` 为 **NULL** | type 分布(更正):其他 56 + 预增 18 / 预减 6 / 首亏 3 / 续亏 2 / 略增 2 / 略减 2 / 扭亏 2 = 35 行属三层内(见 `q2-facts-backfill-2026-07-07.md` V4) | **🔒 上报,待人冻结映射后再定;不设 fallback** | **C3 处置驳回重挂(人 2026-07-07)**:撤销"回退 ann_date"——违裁定原文「任何情况都不得用 ann_date 做 fallback」。**#4 事件日锚 = `first_ann_date`,无 fallback 分支。** 核验:type→#4 三层(预喜/预亏/扭亏)映射 **docs 未冻结** → 上报,映射规则由人批准冻结后再关。冻结后:56 行 type='其他'若判层外 = 非 #4 事件、零口径特例可关;但另 **35 行为实层类型、属 #4 事件却缺锚**,须单独处置(无锚即排除,不得补 ann_date),**不能以"其他层外"一并关闭** |
...
| L3 | **减持预披露 PDF 正文解析件**(#3 事件源结构化字段) | Q2 巨潮覆盖自查(cninfo.py 只抓列表元数据,不解析正文,见 `q2-cninfo-coverage-2026-07-07.md`) | **切片 2 验收之后**,增补项文本随 **v1.5 批复**正式入施工清单,此前不动工 | **人裁档位二(2026-07-07)**:PAP #3 冻结定义含"减持比例≥总股本 1%"门槛,须从预披露公告 **PIT 读取**,不得用 holdertrade 事后数据反筛(前视)→ 解析件为真需求。**范围钉死:仅减持预披露类公告;仅抽 3 字段=股东名/拟减持比例上限/减持期间;解析失败行如实标注;不做通用解析框架。** | 🔒 待 v1.5 入清单后动工 |
