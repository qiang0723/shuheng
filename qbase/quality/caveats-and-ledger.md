...
| C3 | `forecast_snap` 91 行(0.07%)`first_ann_date` 为 **NULL** | type='其他'居多,多 2012–13 早期;`ann_date` 100% 非空(见 `q2-facts-backfill-2026-07-07.md` V4) | **锚定回退+标注**(默认) | #4 事件日锚 `first_ann_date`;为空则**回退 `ann_date` 并显式标注**,不静默当作正常锚。改动需人拍 |
