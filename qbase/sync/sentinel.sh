...
CURN=$(Q "SELECT count(*) FROM public._sentinel_selftest")
if [ -f "$BASEF" ]; then
  BASE=$(awk -F'\t' '$1=="_sentinel_selftest"{print $2}' "$BASEF")
  if [ -n "${BASE:-}" ] && [ "${CURN:-0}" -lt "$BASE" ]; then
    ALERTS+=("❗append-only 行数下降 _sentinel_selftest ${BASE}→${CURN}")
  fi
fi
...
LEVEL="🟢 正常"; [ ${#ALERTS[@]} -gt 0 ] && LEVEL="🔴 需复核"
{
  ...
  echo "— 审计事件触发器: ${ET:-0}/2 在岗 | 冻结触发器: ${FT:-0} | qbase_app→audit权限: ${APPAUD:-?}"
  echo "— _sentinel_selftest 行数: ${CURN:-?}(基线 ${BASE:-未设})"
