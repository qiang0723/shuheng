...
## 8-bis. 复审前时效性确认(补充条目,人令 2026-07-12 追加;§1–8 原文未动)

架构评审侧对"合成小样本操作化读法"两条待答(隔离机制/时效性)的处置留痕:隔离机制三层证据(代码面零 DB 依赖/环境面零 DSN 凭据/权限面触发器独立兜底)**追认通过**;时效性按人令执行**窄口径重跑**(冻结令唯一例外,只读、零代码变更、零新 commit)——在与总表锚点 `8b36eeb` 代码面逐字节零差异的工作树(HEAD `d545c2a`,diff=ops/STATE.md 1 行)实测:

| 腿 | 重跑实测(2026-07-12 深夜) | 与原结果 |
|---|---|---|
| 合成腿(AWS,fixture 现场重生成) | result sha256 `3116ba9b74f7c53b94082c93a476df2257d7a28eae2ad1faa0665b63716a4c22` | 逐字节一致 |
| 焊死环境腿(aliyun 生产 venv,verify_integration) | 7/7 PASS(S1 复用 manifest #2 未增行/S4 n_valid=107 NOT_SIG/S6 双跑 sha `3bef1f81…` 同/S7 台账 25 行全等) | 逐项一致 |

据此"合成小样本操作化读法"**审定通过**,连同本总表转第二独立视角复审。**转复审对象**=总表(锚点 commit `8b36eeb`)+ 三项批复留痕(commit `d545c2a`)+ 本条时效性重跑记录。
