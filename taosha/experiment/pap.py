...
def validate_strategy_execution(se) -> None:
    ...
    if prof == "close_to_next_open":
        ...
        missing = [k for k in PRECLOSE_TO_TAIL_REQUIRED if not str(se.get(k) or "").strip()]
        if missing:
            raise ValueError(
                f"修法#1: preclose_to_tail 缺必填结构字段 {missing}"
                "(截止时点/代理价规则由人在 PAP 冻结前选定,代码不得自行默认)")
        if "close_confirmed" in str(se.get("decision_cutoff")):
            raise ValueError("修法#1: preclose_to_tail 决策必须发生在成交之前"
                             "(decision_cutoff 不得为 close_confirmed)")
    else:
...
if __name__ == "__main__":
    ...
    _ok_pct = {"execution_profile": "preclose_to_tail", "decision_cutoff": "人定截止时点占位",
               "decision_price_source": "人定代理价占位", "fill_window": "尾盘窗占位",
