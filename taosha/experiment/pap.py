...
def canonical_pap_sha256(pap: dict) -> str:
    """冻结 PAP 内容 canonical digest(回修三·窄阻塞一 P1-4,人转外部复核令 2026-07-17)。

    canonical 串 = json.dumps(实质键, ensure_ascii=False, sort_keys=True〔词典序排序键〕,
    separators=(",", ":")〔紧凑分隔符〕);digest = sha256(UTF-8(canonical串) + b"\\n")
    〔末尾单换行〕。与冻结 PAP 文件对账:冻结 PAP 文件即 canonical 串+末尾单换行的字节本体,
    故本函数对文件解析结果重算 == 文件 sha256(v2 实测 == `2611be36…`)。
    实质键 = 顶层剔除 "_" 前缀运行时注入键(_family_trial 等非 PAP 字段,不进 digest)。
    权威性:digest 只由引擎对实收 PAP 内容确定性重算,调用方不得自由填写;调用方另传
    digest 仅用于与重算值逐字断言(runner.run_study pap_sha256_assert),不一致 fail-closed。"""
    core = {k: v for k, v in pap.items() if not k.startswith("_")}
    s = json.dumps(core, ensure_ascii=False, sort_keys=True, separators=(",", ":"))
    return hashlib.sha256(s.encode("utf-8") + b"\n").hexdigest()
