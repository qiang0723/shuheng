...
| `taosha/engine/cleaning.py`+`survivors.py`+`runner.py` | ✅回修单元施工毕(C2 乙案裁定):`st_policy`+`verdict_policy`+`nfv_structured` 三参数显式化,默认=既有行为(合成 e2e 3116ba9b 逐字节+集成 63e2c9fc 改前改后同) | ✅ |
| `taosha/engine/report.py` | ✅回修单元施工毕(C6):NFV 水印段+键名回退渲染(有键才渲染,默认零回归) | ✅ |
| `taosha/harness/verify_limit_open_engine.py` | ✅新增 32/32(顺延边界/st两态/判决隔离/NFV扫描/默认零回归) | ✅ |
