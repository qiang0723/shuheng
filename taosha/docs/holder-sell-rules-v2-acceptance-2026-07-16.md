...
## §11 persist 闭卷留痕(2026-07-17,人令=外审验收通过+persist 授权)

- **外审取证(persist 前,2026-07-17 午后)**:三件原物传输前秘扫 13 类模式零命中;实物包+SHA256SUMS
  交付 AWS `~/shuheng/s7_acceptance_delivery_2026-07-17/`(result `b5edac5d…`/report `3d01ffec…` 与已报全等,
  run7.log `4d5b062f…`);三库证=manifest 87 落库已发布(taosha 权威行+qbase mirror 同 digest+publication_id 18
  attested)/exp4=frozen·空·空/台账 25 行(reg18/froz3/done3/closed1);结构核对全过。三窗字面表述人更正:
  main_window=[0,+4] 唯一进 verdict/secondary_windows=[0,+19] 仅报告/robust_window=[0,+59] 稳健窗,
  实物==冻结设计,不构成偏离。
- **前置断言 4/4 PASS**(2026-07-17 16:15:15+08):①result.json SHA 仍 `b5edac5d…cf565` ②exp4 仍
  frozen/result 空/done_at 空 ③manifest 87 已发布 digest `21e9095e…` ④台账仍 25 行。
- **persist 执行**:既有状态机路径 `ledger.start_running(4)` → `ledger.finish(4, result)`,同一连接同一
  事务末尾一次提交(psycopg 非 autocommit),身份 taosha_app;result 唯一来源=已验收原文件
  `/root/s7run/exp4_result.json`,零重跑/零重生成/零手工改写/零旁路 SQL;脚本留档 `/root/s7run/persist_exp4.py`。
  COMMIT OK @2026-07-17 16:15:33+08。
- **persist 后核验全 PASS**(16:16:01+08,只读):
  - exp4:status=**done**,done_at=2026-07-17 16:15:33.180739+08,顶层 verdict=**NOT_SIG**,
    manifest 87,事件 12,042,N_valid 9,003,敏感性块+三窗块随原 result 完整落库(键探测 t/t);
  - 库内 result_json 与原文件解析后 **逐项全等(parsed_equal=True)**,canonical sha256(排序紧凑 UTF-8)
    双侧同=`a5340f0ad55c363e100a4541b24dfc75eb3cc50e6a79b949837d562f0eaecadd`;库侧
    sha256(result_json::text)=`31feb4162db3f983f09e26c315a7fafb8bf1325c8f299a42fa3091093095596e`
    (jsonb 规范化文本口径,addendum 007 同式);
  - 台账 25 行:**registered 18 / frozen 2 / done 4 / closed 1**(==令预期);
  - manifest 87 三处 digest 未变(权威/镜像/attested 均 `21e9095e…efcd`);
  - 三件原始产物 SHA 未变;生产 git HEAD `fba7168` porcelain 净,零代码改动。
- **密封边界(按令四事实记)**:密封预判载体位于 architecture 仓,施工侧不可见;本次仅持久化已验收
  统计结果,未向 result_json 补写密封内容。
- 台账写入仅此一笔(exp_id=4 终态),无新增 experiment 行。exp4 holder_sell#1 就此闭卷,转停工交终签。

### §11-bis 终签补充事实口径(人,2026-07-17,原文即口径)

> 密封:失败(流程门未接线),校准读数按人裁定记缺失,不补填。

exp4 至此正式归档,不再追加复核、施工或结果解读。
