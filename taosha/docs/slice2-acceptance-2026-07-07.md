...
> ⚠ **口径警示(防翻档误读)**:下列一切数字(剔除率 **0.2708**、N_eff=35、各年剔除数、CAAR、检验统计量、verdict=SIG)均为**合成 fixture 的构造值**——由 `make_ashare_fixture` 按确定性种子人工注入(停牌/一字板/覆盖不足/ST/τ=0 注入 +3% AR)后引擎复算而得,**用途仅为证明引擎路径跑通与口径实现正确,不是任何真实标的的实测体检、不构成任何统计结论**。真实数据体检留切片3。**尤其 0.2708 剔除率是注入构造出来的、非市场实测剔除率**,勿据此推断真实数据的样本损耗。
...
| **7** | 停牌剔除、剔除率按年份分解、>5% 告警 | 现跑剔除率 0.2708(**合成构造值,非实测体检**,见上口径警示)触 ⚠告警;按年份分解 2020/21/22/23 逐年剔除数+原因(suspension/coverage/st)已出;`engine/cleaning.py` 落剔除逻辑 |
