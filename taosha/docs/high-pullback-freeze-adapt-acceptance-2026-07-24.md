...
## 8. 附页(2026-07-24 七):§6.1 已知面已裁定闭合

§6.1(tau_axis 通用文案)已由人裁定闭合(裁定令留痕 `1217d43`,选 C;施工 `aba4336`):
仅 exp11 报告路径将 tau_axis 展示中「首个可交易日」改为「事件后首个有真实bar的价格观察日」,
「可交易口径净收益」段及注记原样;上一令「完整 exp11 报告『可交易』零命中」断言经人显式作废;
一字板句以冻结 PAP 原文为唯一权威;个案展示修正不建通用覆盖机制。
详见 `high-pullback-report-term-receipt-2026-07-24.md`。§6.2(recon 锚 212)由同批人令第 2 条确认闭合。
