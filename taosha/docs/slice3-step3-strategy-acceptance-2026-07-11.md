...
- **事件级收益分布(N=17929,成本乘式)**:净收益 均值 **−0.00861** 中位 −0.03037 胜率 0.250
  sd 0.11489;毛收益 均值 −0.00513;池同跨度 BH 均值 +0.00048;**BHAR(净−基准)均值 −0.00909**
  中位 −0.02147 胜率 0.292 sd 0.09544。
- **四件套② SBHAR 截面 ADJ-BMP**:ρ̄=0.21966(行业内 339,337 对,与事件版 0.2197 同函数同集一致);
  SBHAR 均值 −0.4009 sd 1.1411,z=−47.038 × KP 因子 0.014075 → **adj_z=−0.662,NOT_SIG 标注**
  (双侧 α=0.025 临界 ±2.241;family_trial=2)。朴素 z 显著而 KP 校正后不显著 = 与事件版同构的
  聚集假阳性形态(统计事实陈述)。
- **四件套③ 右偏稳健项**:BHAR skew=+4.669(右偏确认);t_plain=−12.749 → **t_sa=−10.948**
  (Hall 变换右偏修正方向一致)。
