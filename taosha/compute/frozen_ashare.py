...
# ⚠ 语义标签(裁定 A,2026-07-07):以下两窗**非检验窗**。各假设检验窗从 pap_json 读取
#   (事件窗属事件定义、台账为唯一事实源,见 runner);这两值保留、降格为**删失诊断分解窗
#   (R5 本义)**——对全部假设通用、与检验窗并行输出、互不替代。
#   FROZEN dict 内字符串仍历史措辞"主窗/稳健窗"(**不动以保 audit_digest=c795b21e 切片2
#   item10 冻结实证**);语义读法以本注 + item8 改读留痕为准(S2-DEC3 τ=0:=T+1 锚定无过)。
EVENT_WINDOW_MAIN = (0, 2)        # 删失诊断窗 τ=0..2 (=T+1..T+3);历史标签"主窗"
EVENT_WINDOW_ROBUST = (0, 5)      # 删失诊断窗 τ=0..5 (=T+1..T+6);历史标签"稳健窗"
