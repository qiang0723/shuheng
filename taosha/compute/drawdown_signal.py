...
F-rev1 状态机(变更条 = C 案;原 #3(站上水平)/#6(无超时)等不动):
  ...
  · **两正交谓词(谓词归位,人核裁定)**:①站上 = **纯 ma10 谓词** close>ma10(水平,等号归破);
    ②破 ma20 = **独立谓词** close<ma20。二者在状态机层合取(站上 且 未破 ma20 → 计数),不合并成单判据。
  · 在回撤区内连续 3 日「站上 且 未破 ma20」→ 第 3 日=进场 T;
    **破 ma20(close<ma20)/破 ma10(¬站上=close≤ma10)= 计数清零、不锁区**(仍在回撤区、后续满足即可再累计)。
...
FROZEN = MappingProxyType({
    ...
    "station": "纯 ma10 谓词 close>ma10(水平,等号归破)",
    "break_ma20": "独立谓词 close<ma20(破20日线);状态机层清零、供 D3;与站上正交(谓词归位)",
    "reset": "¬站上(破ma10)或 破ma20 → 计数清零、不锁区(仍在回撤区可再累计)",
    "diagnostics": "D1 消费纯 ma10 谓词(¬站上=跌破ma10)、D3 消费纯 ma20 谓词(破ma20);不共享合成判据",
...
def _is_station(c: float, ma10: float) -> bool:
    """站上判据(F-rev1 第3条)= **纯 ma10 谓词**:`close > ma10`(等号归破:`close ≤ ma10` → 非站上)。
    **谓词归位(2026-07-08 人核裁定)**:「站上」与「破 ma20 清零」是**两个正交条件**,不得合并——
    D1 诊断消费本纯 ma10 谓词(进场前是否跌破 ma10 = ¬站上);破 ma20(`close < ma20`)为**独立谓词**、
    在状态机层施加清零、供 D3 消费。二诊断不共享合成判据(防"善意改写"污染 D1 定义)。"""
    return c > ma10
...
def generate_entries(closes: list) -> list:
    ...
    for i in range(n):
        ...
        if episode_start < 0:                 # 新 episode 起
            ...
            broke_ma20 = False
            consec = 0
        # 两正交谓词(谓词归位,人核裁定):站上=纯 ma10、破 ma20=独立谓词
        station = _is_station(c, ma10)        # 纯 ma10:close > ma10
        broke20 = c < ma20                    # 纯 ma20 谓词(独立,供 D3 + 清零)
        # episode 内诊断记账:D1 消费纯 ma10 谓词(跌破 ma10 = ¬站上);D3 消费纯 ma20 谓词(不共享)
        if not station:
            broke_ma10 = True
        if broke20:
            broke_ma20 = True
        if need_escape:                       # #7:进场后未脱离 → 不累计、不进场
            consec = 0
            continue
        if station and not broke20:           # 站上 且 未破 ma20 → 计数(两正交条件在状态机层合取)
            consec += 1
        ...
        else:                                 # 破 ma10(¬站上)或 破 ma20 → 计数清零、不锁区
            consec = 0
    return out
...
if __name__ == "__main__":
    ...
    # R5 谓词归位 + ma20 正交清零(人核裁定 2026-07-08):站上=纯 ma10、破 ma20=独立谓词,D1/D3 不共享合成判据。
    #   ① 判据层:站上 = 纯 close>ma10(等号归破);分歧场景 ma10<close<ma20 → 站上**成立(True)**,
    #      同日破 ma20(close<ma20)为独立谓词 → 状态机层清零、D3 记账、D1(¬站上=跌破ma10)**不受污染**。
    assert _is_station(85.0, 84.0) is True, "R5:站上=纯ma10,ma10<close<ma20 站上成立(True)"
    assert _is_station(84.0, 84.0) is False, "R5:close=ma10 破ma10(等号归破)→ 非站上"
    #   D1/D3 正交:ma10<close<ma20 日 → ¬站上=False(D1 不记跌破ma10)、破ma20=True(D3 记)——两谓词独立
    _c, _m10, _m20 = 85.0, 84.0, 90.0
    assert (not _is_station(_c, _m10)) is False, "R5:ma10<close<ma20 未跌破ma10 → D1 不污染(正交)"
    assert (_c < _m20) is True, "R5:同日破ma20独立谓词成立 → 状态机层清零 + D3 记账"
    #   ② 序列层:破ma20日回踩须清零重计(非跳过)→ 末日进场(reset非skip)、D3=True。
    _seq = ([100.0] * 60 + [100 - k for k in range(1, 21)]   # 平台+缓跌到 80
            + [80.0] * 30                                      # 横盘让 ma20 落到 80
            + [81.0, 82.0, 79.0, 82.0, 83.0, 84.0])           # 站,站,回踩清零,站×3
    _ev5 = generate_entries(_seq)
    assert len(_ev5) >= 1 and _ev5[-1].d3_broke_ma20_before_entry is True, "R5:回踩清零重计+D3记账"
    ...
    print("drawdown_signal.py(F-rev1)自检 OK:R1死锁序列进场 / R2链式防护(仅1) / "
          "R3脱离后再进场 / R4失效不锁区+D3 / R5 谓词归位:站上纯ma10+破ma20正交清零+D1D3不共享(判据+序列) / PIT满窗")
