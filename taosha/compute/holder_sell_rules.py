...
# A6 并披指示(词序任意;裁决表门兜底,表外 fail-closed)。"预披露"之外并收
# "暨后续/及未来减持计划"式无"预披露"字面的新计划披露指示(A2 原文="且不含新计划披露指示"
# 才归进展;含指示者=并披形态,不在人框定 12 行核验集内 → fwp_not_adjudicated fail-closed 留痕)。
_RE_PREDISCLOSE = re.compile(r"预披露|(新|后续|未来)[^，。；]{0,12}减持")
