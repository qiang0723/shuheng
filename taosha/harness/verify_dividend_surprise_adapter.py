"""exp19 driver/report/reader 适配攻击 fixture；零数据库、合成收益。"""
...
from taosha.harness.run_dividend_surprise_study import (
...
PAP_PATH = os.path.join(ROOT, "taosha", "docs",
                        "dividend-surprise-pap-final-2026-08-06.json")
SQL_PATH = os.path.join(ROOT, "qbase", "sql", "026_dividend_surprise_reader.sql")
...
fixture_selection = {
    "events": [
        {"ts_code": "000001.SZ", "event_date": dt.date(2021, 4, 1), "direction": "up"},
        {"ts_code": "000002.SZ", "event_date": dt.date(2021, 4, 2), "direction": "down"},
    ],
    "counters": {**REFERENCE, "input_rows": 2, "implementation_backfill_hits": 0,
                 "full_period_exact_boundary": 0, "research_classification": {},
                 "research_exact_boundary": 0, "directed_group_rows": 2,
                 "event_key_duplicate_groups": 0, "direction_conflict_groups": 0,
                 "initial_multiple_identical_groups": 0,
                 "initial_multiple_conflict_groups": 0,
                 "initial_required_invalid_groups": 0},
    "classification_yearly": {},
    "identities": {"group": True, "full_classification": True,
                   "research_classification": True, "event": True, "direction": True},
    "selection_sha256": "fixture",
}
events = event_rows(fixture_selection, "fixture")
check("A4 EventRow方向层与公告锚",
...
check("A4 选择审计五恒等式", all(audit["identities"].values()), True)
check("A4 source375正式冒充fail-closed", "不得冒充" in failure(
    lambda: assert_formal_snapshot(375)), True)
check("A4 exp19自有manifest ID放行", failure(lambda: assert_formal_snapshot(999)), "")

with open(SQL_PATH, encoding="utf-8") as handle:
    sql = handle.read()
check("A5 current/snap专属视图俱在",
      ("explore_reader_dividend AS" in sql, "explore_reader_dividend_snap AS" in sql),
      (True, True))
check("A5 snap经manifest路由且holdout排北焊死",
      ("study_snap_batch('dividend')" in sql,
       sql.count("ann_date<DATE '2024-07-01'") == 2,
       sql.count("ts_code !~ '\\.BJ$'") == 2), (True, True, True))
check("A5 最小列面与append-only三类破坏保护",
      ("cash_div_tax" in sql, "BEFORE UPDATE OR DELETE" in sql,
       "BEFORE TRUNCATE" in sql, sql.count("GRANT SELECT") == 2),
      (True, True, True, True))
...
_, error = render_attempt({"audit": {"dividend_surprise_selection": {}}})
check("A6 缺快照锚fail-closed", "exp19缺真实StudySnapshot锚" in error, True)
...
with tempfile.TemporaryDirectory() as td:
    ...
    check("A7 exp19真锚标题", rendered.splitlines()[0],
          "═══ 淘沙 · 事件研究体检报告(exp19 年度分红预案大幅变动·signed事件版)═══")
    check("A7 llm/prescreen水印在场",
          "family=dividend_surprise trial=1 source=llm power=prescreen" in rendered, True)
    check("A7 五裁定漏斗与signed单判决说明在场",
          ("A1/B2-P1/C1/D1/E1事件生成漏斗" in rendered,
