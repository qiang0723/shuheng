...
- ~~[ ] 27G/30 份历史密文:Mac 从本 AWS 机 rsync 拉走~~ **作废(改判记 #9):历史份不迁,Mac 初始取数=直接跑 offsite_pull_mac.sh 从阿里云拉现存 5 份轮转至 3 份**
- [ ] Mac 侧每日拉取部署:脚本已入仓=`qbase/sync/offsite_pull_mac.sh`+launchd 模板 `com.shuheng.offsite-pull.plist`(04:00 触发;留存 3 份,改判记 #9;补拉=每次 rsync 源端全部现存密文;⚠阿里云本地仅 5 份→连续漏跑 >4 天产生不可自愈缺份,长关机后手动跑);待人在 Mac load
