...
最后更新:2026-07-07(**切片3 已开工但即刻停**:体系原则新增〔北交所全体系不做〕,核查宇宙+snap **有 328 只 .BJ**→按分支②停切片3,待人裁排除口径(视图层WHERE/宇宙层剔除,建议A视图层)。Q3 已签收 `ea59367`、切片2 终签 `b7e3b4b`。引擎 run 未启动、无作废,停在入口归因阶段)
...
- **⚠ 停切片3(分支②):有北交所→停,待人裁排除口径**=视图层WHERE(建议A:explore_reader三视图加 `!~ '\.BJ$'`,同holdout焊法、引擎role已焊死物理挡、底表宇宙仍忠实存全328防幸存偏差)vs 宇宙层剔除(弊:append-only不可删、伤宇宙完整)。**未裁前不改视图/不改cleaning/不跑引擎。** 引擎run未启动、无作废。
