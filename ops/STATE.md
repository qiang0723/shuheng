...
7. **跑 #4 market 单跑 → 体检报告**(无建议口吻) + **合成回归硬项**(pap桩喂原短窗逐字节不变 + 切片2 logbench 对台不回归)。**#4=台账 `exp_id=5`(`forecast_drift/业绩预告漂移`,status=frozen,window="T+1起,后20/60日"=(20,60)、benchmark market_hypothesis=全市场等权、event_def=业绩预告 valid_time=first_ann_date、sample_gate=30、snapshot=forecast_snap Q2 batch#1)。步7=读 exp_id=5 校验 frozen 执行,已在令内——不需也不得再登记(旧"#4 pap冻结登记"前置系错认,作废,人 2026-07-08 纠正)。** **§步7 前置(2026-07-08):①✅clean_event 空价行硬化完成**(HEAD 99d07bf:空rows→剔除no_price+入年份报告+偏差声明;修 rows[0] IndexError;合成既有字段逐字节不变、自检并入正向断言)。**②✅industry 缺失→'unknown' 残余组完成**(人批:不猜不补,ρ̄口径④'unknown'单独成组,报告附占比 industry_coverage,>5%事件 escalate升级上报;合成受控diff=仅新增该键)。**③规模(工程自决,红线=不绕视图):** 全事件票 prices ~千万行加载评估,ViewReader() 默认 sample=全事件票。引擎读全市场等权基准=步3 market_return_current(读表不现算)。
