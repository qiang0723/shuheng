...
最后更新:2026-07-07(**切片3 北交所排除五条✅全成(886c708,git真身=库 prices/events .BJ=0)**,转入原开工令续:改cleaning(缺行+calendar判停牌)→接事件源exp_id5→跑#4=第一份真实体检报告。Q3 签收 `ea59367`、切片2 终签 `b7e3b4b`。慢比快好)
...
1. **检验窗从 pap 读 ✅ 步3a 完成(runner 重构 + 合成回归逐字节验证过)**:`pap.parse_test_windows(pap)` 从 window 文本读检验窗(#4=(20,60)/#3=(5,20,60);后N日=τ=0..N-1=N点;解析失败即raise)。runner run_study 解析检验窗→main_len/robust_len 驱动事件窗/per_tau/car/三法/verdict(不再用 fa.EVENT_WINDOW);car taus 标签改检验窗[0,+main_len-1]/[0,+robust_len-1];adj_bmp_by_tau 变长(n_tau=len 自适应)。**合成回归**:synth_pap window 改文本"后3/6日"(=原3/6点),**git stash 改前基线 vs 改后 diff = 逐字节完全一致**(约束③达成,切片2 不回归)。**步3b 删失诊断窗(frozen 3/6)并行输出**——**R5 本义已裁(人 2026-07-07):诊断窗输出三件套**:①各 τ 逐日 AR(截面均值+显著性,看反应时间形状与延迟价格发现);②各 τ 一字板/触板/停牌 计数占比;③①②按**板块四层分拆(强制)**。**全部报告项、不进 verdict。** 依赖步5 cleaning(✅已备)。
...
3. **市场收益预计算落库 ✅ 口径已裁**:全市场等权日收益表**放 taosha(L2)认可**(统计口径产物归 L2),≈8797行,**append-only+batch 溯源**,北交所排除口径,引擎读表不现算;**schema 随验收报**。
4. **建 ViewReader ✅ DSN 预批(人 2026-07-07)**:照方案(taosha_engine 只读密码 + `.env` 600 不进 git);**归类="新机权限配置"非类③**(taosha_engine 是枢衡域内新 role、未触老平台;从严上报姿态对但账目分开)。**role 权限只收不扩**,发现"还需要"=上报不 GRANT。事件票取数(非全宇宙)+读市场收益表+calendar 轴。
