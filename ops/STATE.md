...
最后更新:2026-07-08(**切片3 步3 市场收益落库 ✅ 完成验收**(见 §步3 与 `taosha/docs/slice3-step3-market-return-acceptance-2026-07-08.md`)。引擎逻辑改造大块早已全完成(HEAD 8823580)。**剩真实库接入步4→步7**:步4 ViewReader(DSN配置,已预批)→步7跑#4真实数据=第一份体检报告。已完成:frozen_ashare语义标签裁定A(digest c795b21e不变)、pap检验窗解析、步3a检验窗从pap读(合成逐字节)、步5 cleaning约束②停牌/一字板判据分离、步3b删失诊断窗R5三件套、**步3市场等权日收益落库(taosha market_batch#1/market_eqw_return 8186行)**。北交所排除五条已成(886c708)、Q3签收ea59367、切片2终签b7e3b4b。慢比快好)
